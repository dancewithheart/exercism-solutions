#lang racket

(provide response-for)

(define (question? prompt) (string-suffix? prompt "?"))
(define (blank? prompt) (eq? (string-length (string-trim prompt)) 0))
(define (all-caps? prompt) (equal? (string-upcase prompt) prompt))
(define (has-chars? prompt) (not (equal? (string-downcase prompt) (string-upcase prompt))))

(define (response-for prompt)
  (cond [(and (question? prompt) (all-caps? prompt) (has-chars? prompt))
           "Calm down, I know what I'm doing!"]
        [(question? prompt) (has-chars? prompt) "Sure."]
        [(blank? prompt)    "Fine. Be that way!"]
        [(and (has-chars? prompt) (all-caps? prompt)) "Whoa, chill out!"]
        [else               "Whatever."]))

