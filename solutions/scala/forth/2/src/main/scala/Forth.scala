...
object Token:
  ...
  def isArithOp(x: Token): Boolean = x == Token.Plus || x == Token.Mins || x == Token.Multi || x == Token.Div
...
class Forth extends ForthEvaluator:
  ...
  def evaluate(xs: List[Token]): Either[ForthError, ForthState] =
  ...
  def handleAritmetic(word: Token, stack: mutable.Stack[Token]): Either[ForthError, mutable.Stack[Token]] =
    if stack.size < 2 then ForthError.StackUnderflow.left
    else
      val fst = stack.pop()
      val snd = stack.pop()
